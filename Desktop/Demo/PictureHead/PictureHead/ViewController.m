...
- (void)initHeadIcon
{
    ...
    NSString *filePath = [[paths objectAtIndex:0] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.png", @"保存图片"]];
    self.headIcon.image = [UIImage imageWithContentsOfFile:filePath];
}
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    //保存图片
    [self saveImage:userImage name:@"保存图片"];
...
- (void)upDateHeadIcon:(UIImage *)photo {
    ...
    [manager POST:imageURl parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData> formData) {
        ...
        [formData appendPartWithFileData:UIImageJPEGRepresentation(photo, 1.0) name:@"text" fileName:@"image.jpg" mimeType:@"image/jpg"];
